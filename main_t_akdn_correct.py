...
def evaluate(model, dataloader, Ks, device):
    ...
    with torch.no_grad():
        user_all_embed, item_all_embed = model.get_embeddings()
    ...
    with tqdm(total=len(user_ids_batches), desc='Evaluating Iteration') as pbar:
        for batch_user_ids in user_ids_batches:
            ...
            with torch.no_grad():
                user_embed = user_all_embed[batch_user_ids]
                batch_scores = torch.matmul(user_embed, item_embed.transpose(0, 1))
...
def train(args):
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        # ------------------------------------------------------------------
        # 2. Train KGE (TransR Pairwise Ranking Loss) every 3 epochs
        # ------------------------------------------------------------------
        if (epoch % 3) == 0:
            time_kge = time()
            kge_total_loss = 0
            ...
            logging.info('KGE Training: Epoch {:04d} skipped (run every 3 epochs).'.format(epoch))
