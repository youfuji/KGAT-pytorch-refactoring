"""
T-AKDN 勾配フロー可視化スクリプト

main_t_akdn.py と同じデータ読み込み・モデル構築を行い、
指定エポック数だけ学習した後の勾配フローを分析する。

使い方:
  python visualize_gradient_flow.py --data_name alibaba-fashion --n_warmup_epochs 0
  python visualize_gradient_flow.py --data_name alibaba-fashion --n_warmup_epochs 5 --use_pretrain 1
"""
...
def analyze_intermediate_gradients(model, data, device):
    ...
    for name, tensor in intermediates.items():
        if tensor.is_sparse:
            print(f"  {name:<35} {'(sparse)':<20} {'N/A':<8} {'---':<14} {'---':<14} {'---':<14} {'---':<14} ℹ️  sparse tensor")
            continue
...
# ============================================================
def visualize_computation_graph(model, data, device, save_path='gradient_flow_graph'):
    """
...
def main():
    ...
    # parser_t_akdn のパーサーを拡張
    parser = argparse.ArgumentParser(description="T-AKDN Gradient Flow Visualization")

    # main_t_akdn.py と同じ引数
    ...
    parser.add_argument('--graph_save_path', type=str, default='gradient_flow_graph',
                        help='torchviz 出力の保存パス (拡張子なし)')

    args = parser.parse_args()

    # save_dir (ログ出力には不要だがデータローダー互換性のため)
    args.save_dir = '/tmp/gradient_viz_logs/'
    args.n_epoch = args.n_warmup_epochs  # ウォームアップ用
